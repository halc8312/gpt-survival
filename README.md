# 星屑コロニー / Stardust Colony
 ...
 GitHub Pages 向けの静的ブラウザゲーム MVP です。Phase 0–4 の技術基盤として、データ/アセット読み込み、64×64 のアイソメトリック地形表示、カメラ操作、タイル/資源選択、資源ノード描画、建築ゴーストプレビュー、建物配置の基礎までを実装しています。
...
建築ゴーストは建築モード中にマウスホバー中のタイルを左上基準に表示されます。配置可能なら緑系、不可ならオレンジ/赤系で表示され、不可理由はデバッグオーバーレイで確認できます。初期状態ではマップ中心付近に `building_crash_core` が配置されます。

## Known limitations

- Phase 4 の範囲のみ実装しています
